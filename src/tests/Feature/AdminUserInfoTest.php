...
class AdminUserInfoTest extends TestCase
{
    ...
    public function test_admin_can_see_all_staff_names_and_emails_on_staff_list(): void
    {
        ...
        // ✅ 「詳細」リンク（スタッフ別勤怠ページ）
        // routes: /admin/attendance/staff/{id} かつ name('admin.attendance.staff')
    ...
    public function test_admin_can_view_selected_staff_monthly_attendance_list_and_data_is_correct(): void
    {
        ...
        $res = $this->actingAs($this->admin)
            ->get(route('admin.attendance.staff', [
                'id'    => $this->staff1->id,
                // ルートパラメータに無いものはクエリとして付与される（?month=2025-12）
                'month' => '2025-12',
            ]));
        ...
        // 休憩/合計（StaffController の fmtMinutes 表示想定）
        $res->assertSee('1:00');
        $res->assertSee('7:00');

        // ✅ 管理者の勤怠詳細は admin.attendance.detail（/admin/attendance/{id}）
